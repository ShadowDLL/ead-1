<?php
    ...
    if (!empty($_POST['video'])){
        ...
        try{
            $conexao = conn_mysql();
            $SQLInsert = 'INSERT INTO material (nome, embedded, unidade_id)
                                                  VALUES (?,?,?)';
            //prepara a execução
            $operacao = $conexao->prepare($SQLInsert);					  
            //executa a sentença SQL com os parâmetros passados por um vetor
            $inserir = $operacao->execute(array($nome, $video, $aula));
            ...
            if ($inserir){
                ...
                echo "<h1>Material cadastrado com sucesso.</h1>\n";
        ...
        if (move_uploaded_file($_FILES['file']['tmp_name'], $_UP['pasta'] . $nome_final)) {
          try{
                ...
                $SQLInsert = 'INSERT INTO material (nome, unidade_id, filepath, tipo)
                                                      VALUES (?,?,?,?)';
                //prepara a execução
                $operacao = $conexao->prepare($SQLInsert);					  
                //executa a sentença SQL com os parâmetros passados por um vetor
                $inserir = $operacao->execute(array($nome, $aula, $_UP['pasta'].$nome_final, $extensao));
                // fecha a conexão ao banco
                $conexao = null;
                //verifica se o retorno da execução foi verdadeiro ou falso,
                //imprimindo mensagens ao cliente
